...
vector<PSV> First1;	//存放first集合的中间结果 
vector<PSV> First;	//存放最后first集合 
vector<PSV> Follow;	//存放Follow1集
vector<PSV> Follow1;//存放follow集的中间结果 
vector<PSV> Select;	//存放select集
...
//输出pair<pair<>>的内容
void Displaypsss(vector<PSSS> v)
{
...
void get_first_pre()
{
	...
	for(int i=0;i<cutgrammar.size();i++)
	{
		...
		head_exist(head,tmp,First1);
	}
}
//判断最后的First1集合是否已经存在该行文法 
...
void get_first()
{
	...
	while(n!=First.size()){
		for(int i=0;i<First_tmp.size();i++)
		{
			...
			if(flag==0) First.push_back(First_tmp[i]);
		}
	} 
}
//处理每行文法所有的符号 
void get_follow_continue(string head,vector<string> sym)
{
	...
	for(int i=0;i<sym.size()-1;i++)
	{
		...
		if(jbs(s1[0])){
			if(jbs(s2[0])){
				for(int m=0;m<Follow1.size();m++)
				{
					if(Follow1[m].first==s1){//s1符号已经在Follow1集中，可能不完整
						for(int n=0;n<First.size();n++)
						{
							if(First[n].first==s2){
								...
								for(int o=0;o<v3.size();o++)
								{
									if(v3[o]!="#"){
										int flag2=0;
										vector<string> v4=Follow1[m].second;
										...
										vector<string> v5=Follow1[m].second;
										v5.push_back(s2);
										Follow1[m].second=v5; 
									}
								}
				...
				//cout<<s2<<"  AAA"<<endl;
				int flag=0;
				for(int j=0;j<Follow1.size();j++)
				{
					if(Follow1[j].first==s1){
						flag=1;
						vector<string> v1;
						v1=Follow1[j].second;
						int flag1=0;
...
//去掉follow集中没有意义的内容，和左部一样的内容 
void handle_follow(vector<PSV> &vec)
{
	for(int i=0;i<vec.size();i++)
	{
		string s=vec[i].first; 
...
void remove_common()
{
	...
	{
		vector<string> v=Follow[i].second;
		vector<string> res;
		for(int j=0;j<v.size();j++)
		{
			int flag=0;
...
{
	vector<PSV> Follow2; 
	copypsv(Follow2,Follow1);
	handle_follow(Follow2);
	int n=Follow2.size();
	while(n!=Follow.size()){
		int flag=0;
		for(int i=0;i<Follow2.size();i++)
		{
			string s=Follow2[i].first;
			vector<string> v=Follow2[i].second;
			vector<string> v1;
			for(int j=0;j<v.size();j++)
			{
				if(jbs(v[j][0])){
					...
					{
						if(Follow[k].first==v[j]){
							flag1=1;
							vector<string> v2=Follow[k].second;
							for(int l=0;l<v2.size();l++)
								v1.push_back(v2[l]);
						}
					}
					if(flag1==0) v1.push_back(v[j]);
				}
	...
	for(int i=0;i<Select1.size();i++)
	{
		...
		for(;j<s.length();j++)
		{
			if(s[j]!=':'&&s[j-1]==':'){
				...
				if(jbs(sr[0])&&s[j]=='\'') sr+=s[j];
				else if(jss(sr[0])&&jss(s[j])) sr+=s[j];
				else break;
			}
		}
		//依次处理每行文法
		...
		if(jbs(sr[0])){
			...
			{
				string s1=First[k].first;
				if(s1==sr){
					vector<string> v=First[k].second;
					for(int l=0;l<v.size();l++)
					{
						if(v[l]=="#") flag1=1;
						else res.push_back(v[l]);
					}
				}
			}
		}
		...
		if(flag1){	//即能推出空 
			for(int k=0;k<Follow.size();k++)
			{
				string s2=Follow[k].first;
				if(s2==head){
					vector<string> v1=Follow[k].second;
...
void analysis_table()
{
	...
	for(int i=0;i<table.size();i++)
	{
		...
		{
			string s1=Select[j].first;
			if(s1==s){
				vector<string> v=Select[j].second;
...
void output()
{
	...
	for(int i=0;i<Table.size();i++)
	{
		string s1=Table[i].first.first;
		string s2=Table[i].first.second;
		string s3=Table[i].second;
		fout<<s1<<' '<<s2<<' '<<s3<<endl;
...
int main()
{
	...
	get_follow_end();
	cout<<"完整的Follow集："<<endl;
	Displaypsv(Follow);
	
	get_select();
	cout<<"完整的Select集："<<endl;
	Displaypsv(Select);
